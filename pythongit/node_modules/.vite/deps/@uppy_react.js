...
var addFiles = (companionFiles, plugin, provider) => {
  const tagFiles = companionFiles.map((f2) => getTagFile_default(f2, plugin, provider));
  const filesToAdd = [];
...
var ProviderView = class {
  ...
  #searchDebounced = (0, import_debounce.default)(this.#search, 500);
  onSearchInput = (s2) => {
...
var SearchProviderView = class {
  ...
  render(state, viewOptions = {}) {
    ...
    return u("div", { className: (0, import_classnames6.default)("uppy-ProviderBrowser", `uppy-ProviderBrowser-viewType--${opts.viewType}`), children: [opts.showFilter && u(SearchInput_default, { searchString, setSearchString: this.setSearchString, submitSearchString: this.search, inputLabel: i18n("search"), clearSearchLabel: i18n("resetSearch"), wrapperClassName: "uppy-ProviderBrowser-searchFilter", inputClassName: "uppy-ProviderBrowser-searchFilterInput" }), u(Browser_default, { toggleCheckbox: this.toggleCheckbox, displayedPartialTree: this.getDisplayedPartialTree(), handleScroll: this.handleScroll, openFolder: async () => {
...
var J = class {
  ...
  translateBlock(e2, t2) {
    let s2 = I.get(t2), i2 = V.get(t2), n2 = B.get(t2), r2 = this.options[t2], a2 = r2.reviveValues && !!s2, h4 = r2.translateValues && !!i2, f2 = r2.translateKeys && !!n2, l2 = {};
    for (let [t3, r3] of e2) a2 && s2.has(t3) ? r3 = s2.get(t3)(r3) : h4 && i2.has(t3) && (r3 = this.translateValue(r3, i2.get(t3))), f2 && n2.has(t3) && (t3 = n2.get(t3) || t3), l2[t3] = r3;
    return l2;
  }
...
var ee = class extends class {
  ...
  findAppSegmentsInRange(e2, t2) {
    t2 -= 2;
    let s2, i2, n2, r2, a2, h4, { file: f2, findAll: l2, wanted: o2, remaining: u3, options: d3 } = this;
    for (; e2 < t2; e2++) if (255 === f2.getUint8(e2)) {
      if (s2 = f2.getUint8(e2 + 1), Q(s2)) {
        if (i2 = f2.getUint16(e2 + 2), n2 = Z(f2, e2, i2), n2 && o2.has(n2) && (r2 = y2.get(n2), a2 = r2.findPosition(f2, e2), h4 = d3[n2], a2.type = n2, this.appSegments.push(a2), !l2 && (r2.multiSegment && h4.multiSegment ? (this.unfinishedMultiSegment = a2.chunkNumber < a2.chunkCount, this.unfinishedMultiSegment || u3.delete(n2)) : u3.delete(n2), 0 === u3.size))) break;
...
var se = class extends J {
  ...
  parseTags(e2, t2, s2 = /* @__PURE__ */ new Map()) {
    ...
    i2 = new Set(i2);
    let r2 = i2.size > 0, a2 = 0 === n2.size, h4 = this.chunk.getUint16(e2);
    e2 += 2;
    for (let f2 = 0; f2 < h4; f2++) {
      let h5 = this.chunk.getUint16(e2);
      if (r2) {
        if (i2.has(h5) && (s2.set(h5, this.parseTag(e2, h5, t2)), i2.delete(h5), 0 === i2.size)) break;
      } else !a2 && n2.has(h5) || s2.set(h5, this.parseTag(e2, h5, t2));
      e2 += 12;
    }
    return s2;
  }
  parseTag(e2, t2, s2) {
    ...
    if (2 === n2) return "" === (h4 = (function(e3) {
      for (; e3.endsWith("\0"); ) e3 = e3.slice(0, -1);
      return e3;
    })(h4 = i2.getString(e2, r2)).trim()) ? void 0 : h4;
    var h4;
...
var re = Object.freeze({ __proto__: null, default: G2, Exifr: H2, fileParsers: m, segmentParsers: y2, fileReaders: b2, tagKeys: B, tagValues: V, tagRevivers: I, createDictionary: x2, extendDictionary: C, fetchUrlAsArrayBuffer: S, readBlobAsArrayBuffer: A2, chunkedProps: L, otherSegments: T2, segments: z, tiffBlocks: P, segmentsAndBlocks: F, tiffExtractables: j, inheritables: E, allFormatters: M, Options: R, parse: Y });
...
function FileCard(props) {
  ...
  return (
    ...
    u("div", { className: (0, import_classnames9.default)("uppy-Dashboard-FileCard", className), "data-uppy-panelType": "FileCard", onDragOver: ignoreEvent_default, onDragLeave: ignoreEvent_default, onDrop: ignoreEvent_default, onPaste: ignoreEvent_default, children: [u("div", { className: "uppy-DashboardContent-bar", children: [u("div", {
      className: "uppy-DashboardContent-title",
      // biome-ignore lint/a11y/useSemanticElements: ...
      role: "heading",
      "aria-level": 1,
...
function Dropzone(props) {
  ...
  return u("div", { className: "uppy-reset", "data-uppy-element": "dropzone", role: "presentation", children: [u("input", { ...getInputProps(), tabIndex: -1, name: "uppy-dropzone-file-input", className: "uppy:hidden" }), u("div", {
    ...getRootProps(),
...
function UploadButton(props) {
  ...
  } }), u("span", { className: "uppy:relative uppy:z-10", children: ctx.status === "uploading" || ctx.status === "paused" ? `Uploaded ${Math.round(ctx.progress)}%` : ctx.status === "error" ? "Retry" : ctx.status === "complete" ? "Complete" : "Upload" })] }), ctx.status === "uploading" || ctx.status === "paused" ? u("div", { className: "uppy:flex uppy:gap-2", children: [ctx.uppy?.getState().capabilities.resumableUploads ? u("button", { type: "button", "data-uppy-element": "pause-button", "data-state": ctx.status, onClick: () => {
...
var UppyContext = (0, import_react3.createContext)({
  uppy: void 0,
  status: "init",
  progress: 0
});
function UppyContextProvider({ uppy, children }) {
  const [status, setStatus] = (0, import_react3.useState)("init");
...
function useUppyContext() {
  const ctx = (0, import_react3.useContext)(UppyContext);
  if (!ctx.uppy) {
...
function Dropzone2(props) {
  ...
  const ctx = (0, import_react4.useContext)(UppyContext);
...
function useDropzone(options) {
  ...
  const dropzone = (0, import_react10.useMemo)(
...
function useFileInput(props) {
  const ctx = useUppyContext();
  const fileInput = (0, import_react11.useMemo)(
...
function useRemoteSource(sourceId) {
  ...
  const controller = (0, import_react12.useMemo)(() => createRemoteSourceController(uppy, sourceId), [uppy, sourceId]);
...
function useScreenCapture({ onSubmit }) {
  const { uppy } = useUppyContext();
  const controller = (0, import_react13.useMemo)(() => createScreenCaptureController(uppy, onSubmit), [uppy, onSubmit]);
...
function useUppyState(uppy, selector) {
  const subscribe = (0, import_react15.useMemo)(() => uppy.store.subscribe.bind(uppy.store), [uppy.store]);
  const getSnapshot = (0, import_react15.useCallback)(() => uppy.store.getState(), [uppy.store]);
...
function useWebcam({ onSubmit }) {
  const { uppy } = useUppyContext();
  const controller = (0, import_react16.useMemo)(() => createWebcamController(uppy, onSubmit), [uppy, onSubmit]);
  const store = (0, import_react16.useSyncExternalStore)(controller.subscribe, controller.getSnapshot, controller.getSnapshot);
  return store;
}
export {
